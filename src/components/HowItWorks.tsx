...
const HowItWorks = () => {
  return <section className="py-24 bg-primary/5 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="hidden md:block max-w-4xl mx-auto mb-8">
          <motion.div className="h-1 bg-gradient-to-r from-primary/20 via-primary to-primary/20 rounded-full origin-left" initial="hidden" whileInView="visible" viewport={{
          once: true,
          margin: "-100px"
        ...
        margin: "-100px"
          ...
          {steps.map((step, index) => <motion.div key={index} variants={cardVariants} className="relative p-8 rounded-2xl shadow-card hover:shadow-lg transition-all duration-300 border border-border/50 hover:border-primary/20 group bg-secondary">
              {/* Step indicator dot */}
              ...
              {index < steps.length - 1 && <div className="md:hidden absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-0.5 h-8 bg-gradient-to-b from-primary/50 to-transparent" />}
            </motion.div>)}
        </motion.div>
      </div>
    </section>;
